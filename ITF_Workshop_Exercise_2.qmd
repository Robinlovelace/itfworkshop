...
By the end, you will have quantitative evidence you can use to support real transport planning decisions — and working code you can adapt for your own future needs.

The emphasis in this exercise is on **understanding, interpreting, and applying results** — not on coding. To keep things focused, the Python code in this exercise is hidden.

Before each step, a short explanation describes what the code is doing and why. You don't need to read or understand the code itself — just follow the instructions, run each cell, and focus on what the outputs tell you.
...
| 📐 | **Calculate** | Derive useful performance indicators |
| 🗺️ | **Visualise & interpret** | Map results and develop evidence-based policy recommendations |

> 💡 **No coding experience needed.** Just like before, all of the Python code is already written — just follow the step-by-step instructions and run each cell in order.
...
Your city manager was impressed with your work in Exercise 1 and is back with a new request. To support upcoming budget decisions, they need evidence on where traffic is highest across the city — but as before, there is no time or budget to collect survey data.

Your task is to use open-source tools to **estimate travel demand and assign it to the road network**, producing a picture of traffic flows across the city. You will then interpret those results to develop a short set of **evidence-based policy recommendations**.

**By the end of this exercise, your group will produce:**
- A set of maps showing estimated traffic volumes on the city street network
...
## ▶️ Reminder: How to Run the Code

The exercise is divided into **tasks**, each containing one or more blocks of code called **cells**.

1. 🖱️ **Hover** over a cell — a **▶** button appears on the left
2. 👆 **Click ▶** to run it
3. 👀 **Watch** the output appear below the cell
4. ✅ **Wait** for the **"Complete"** message before moving on

> ⚠️ Run each cell **once**, in order, and always wait for **"Complete"** before continuing.
...
To complete your assignment, you decide to use OpenStreetMap data to estimate the total amount of transport infrastructure, and WorldPop data to estimate the population of your assigned city.

You can find more information about each data source here: OpenStreetMap, WorldPop.

In order to work with this data, you will need to install a few special Python "libraries", which contain code written by other people to make it easier to manipulate transport and population data.

Hover over the cell below and click "Run" to install the special libraries.
...
Please choose the same city you used last time. Here's a reminder of the correct names to use:
...
To select your city, find the line that reads `city = "YOUR CITY HERE"` in the cell below and replace `YOUR CITY HERE` with your chosen city name — **spelled and capitalised exactly as shown in the table above**.

For example, to select Dnipro or Donetsk:
...
```python
city = "Donetsk, Donetsk Oblast"
```

▶️ **Once you've made your edit, run the cell.**
...
An **origin-destination (OD) matrix** describes how many trips are made between every pair of zones in a city. It is a fundamental input to any traffic model.
...
This process uses three open-source libraries working together in sequence:
...
> ⚠️ **These cells may take 1–2 minutes to run.** This is normal — the tools are downloading and processing real city data.
...
Before any analysis can begin, this cell loads all the software libraries needed for this task. It also sets the key parameters for the analysis: the area to study (a 5 km radius around the city centre), how finely to divide it (a 12×12 grid of zones), and where to save the results.
...
OUTPUT_DIR = "/content/grid2demand_output"
```

### 🌍 Cell 2: Download data

This cell downloads real data from OpenStreetMap for your chosen city. It starts by pinpointing the city centre coordinates, then downloads the driveable road network within 5 km. Finally, it collects points of interest (POIs) — shops, offices, amenities, buildings and more — which will be used in the next steps to estimate how much travel activity each part of the city generates.
...
Raw map data isn't immediately usable by a transport model — it needs to be cleaned and restructured first. This cell converts the road network into a standard transport format, merges it with the POI data, and then divides the study area into a 12×12 grid of analysis zones. These zones are the spatial building blocks of the OD matrix.
...
This is where the demand estimation happens. The cell uses a gravity model — one of the most established methods in transport planning — to estimate how many trips occur between every pair of zones. The logic mirrors real-world behaviour: more trips are expected between zones with lots of activity that are close together, and fewer between zones that are distant or have little going on. The full OD matrix is saved to your output folder, and a summary is shown below.
...
Before moving on, it is worth taking a moment to inspect the results to ensure that they look reasonable.

In this task, you will visualise the estimated OD demand as **desire lines** — straight lines connecting each origin zone to each destination zone, with line thickness proportional to the number of estimated trips. This is one of the most widely used tools for communicating travel demand patterns to planners and decision-makers.
...
Before moving on, discuss these questions with your group:
...
You now have an estimate of **how many trips** are made between zones. But where do those trips actually go? In this task, you will find out — by routing every estimated trip across the real street network.

---

🧮 **What the model does, in plain English:**

> *"If someone needs to travel from Zone A to Zone B, they will take the shortest path through the street network — and every street along that path gets credited with that trip."*

Repeat this for every OD pair in your matrix, and you end up with an estimated traffic volume on every street in the city.
...
Before any routing can happen, the model needs to work in metric coordinates (metres, not latitude/longitude) so that distances are meaningful. This cell reprojects the street network accordingly, then solves a practical problem: the OD matrix talks about zones, but routing happens between actual locations (buildings and streets). So for each zone, the model finds the nearest intersection and uses that as the entry point into the network — a process called snapping.
...
The raw gravity model produces trip volumes that are not calibrated to Ukrainian cities — the absolute numbers of trips can be unrealistic, even if the spatial pattern (which corridors are busiest) is plausible. To fix this, we rescale the entire OD matrix so that total daily trips matches a realistic estimate based on the city's population.
 ...
 **Enter your city's population in the code box below**. It will look like this:
 ...
 You can use the WorldPop figure from Exercise 1, or a published estimate such as Wikipedia or the State Statistics Service of Ukraine.

 💡 To find your WorldPop figure from Exercise 1: scroll to the output of the population download cell — it will have printed a line reading "Total population in study area: XXX,XXX". Use that number here.

We assume each resident makes 2.1 trips per day on average — a figure drawn from a household mobility survey conducted in Poltava, Ukraine in 2018, which is the best available published estimate for a Ukrainian city. Note that this is a pre-war figure and actual travel rates may vary between cities and have likely shifted since 2022. As with any population-based estimate, it also represents a conservative lower bound: it counts only residents, whereas the real network also serves commuters and visitors from surrounding areas.
...
This is the core of the traffic assignment. For each origin zone, the model calculates the shortest path to every destination zone in a single efficient pass, then adds the estimated trip volumes onto every street segment along each route. By the time every origin has been processed, each street in the network has accumulated a total estimated flow — the sum of all trips routed across it.

> 💡 This approach — routing demand along shortest paths — is known as **all-or-nothing assignment**. It is one of the simplest and most widely used methods in transport modelling, and a natural starting point when detailed behavioural data is unavailable.
...
- Where are the busiest corridors in your city? Is demand concentrated in the centre, or spread across the network?
